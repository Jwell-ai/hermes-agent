...
# -*- coding: utf-8 -*-
"""
generate.py — 把反应 spec 经 reaction_kernel 计算后注入 templates/reaction.html，产出单页交互网页。

数据由 lib/reaction_kernel.py 驱动（单一数据源）：配平、原子守恒、键的断/成、坐标与方程一致。
每个 build_* 返回一个“作者 spec”，assemble_data 负责校验与装配。

依赖 sympy。请使用已配置 EduLab 运行时中的 python3：
...
TEMPLATE = SKILL_DIR / "templates" / "reaction.html"
