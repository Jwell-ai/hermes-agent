...
模板 `templates/lesson.html` 读取一个 JSON 对象，含三部分：`lesson` / `steps` / `model`。
...
### 多语言（lesson.ui，可选）
模板内置中文 `defaultUI` 兜底。输出英文时，把界面文案放入 `lesson.ui`（键见 `templates/lesson.html` 的 `defaultUI`），例如 `previous/next/finish/stepTemplate/sceneLabel/...`，并把 `lesson.language` 设为 `en`。`steps.content` 与 `title` 由模型按目标语言书写。`answerValue` 等 LaTeX 与语言无关。
