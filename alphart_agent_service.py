...
- Prefer a fixed 1920x1080 logical game window/stage, but the page must scale the whole stage to fit the actual browser/iframe viewport. Do not build a page that requires scrolling to see the full game.
- Use a single visible stage/root container with CSS like: html/body width:100%, height:100%, margin:0, overflow:hidden; * box-sizing:border-box; body display:grid/place-items:center; stage aspect-ratio:16/9, width:min(100vw, calc(100vh * 16 / 9)), height:min(100vh, calc(100vw * 9 / 16)), overflow:hidden.
...
- Avoid negative offsets, fixed-position overlays, viewport-sized panels, oversized absolute dialogs, or transforms that push UI outside the game frame.
