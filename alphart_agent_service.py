...
- Required QA acceptance criteria before calling the tool: #game-root is bounded and overflow:hidden; no position:fixed or negative offsets; start/restart are wired; keyboard/mouse/touch input mutates state; requestAnimationFrame/setInterval or update/render loop exists; score/progress/level/timer/lives/health visibly changes; win/fail/completion can be reached by playing.
